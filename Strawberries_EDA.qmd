...
```{r}
# There are a lot of NA values in this dataset. Let's remove them and see what we get from the rest:
...
ggplot(diff_state_prices, aes(State, market_diff, color = Year))+
  ...
  labs(title="Yearly variations in Fresh vs Production Market Price by State",
...
g<-ggplot(data=prices, aes(Year, Value, color=Purpose))+
  geom_point()+suppressWarnings(stat_smooth())
suppressWarnings(print(g))
```

There is a limited amount of information we can glean from this - this is reflected in the increasing area of the error 'shadow' around the line. In large part this is due to the dwindling number of points each consecutive year. All the same, we can see that in none of the years do any of the fresh market prices go beneath those of even the highest production market. The overlap of the shadows towards the end might indicate that perhaps we might find a point or two like that if we had data for more recent years. That is certainly interesting. Were that to be true, what would drive that reversal of the dynamic of the two prices?

Maybe there's a world(not this world and not a world we can infer from the data above) where people buy fewer fresh strawberries. Why? Let's look at some analysis of Chemical data to see if there are any reasons to avoid strawberries.
...
Starting with per year. If we try and plot all of our pesticides, it becomes impossible to glean anything clear. There are a few pesticides that are used in small quantities so let's not look at those.
